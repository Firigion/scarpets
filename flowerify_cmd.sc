...
__under_player_flowers(range) -> (
	pp = pos(player()); 
	i = first(range(pp:1), block(pp:0, (pp:1) - _, pp:2) =='grass_block');
	if(i != null,
		one_pass(pos_offset(pp, 'down', i), range);
	);
);

global_flowerify = false;

set_flowerify_continuous(bool_value) -> global_positions = bool(bool_value);

flowerify_continuous(dx, dy, dz) -> if(global_positions, __under_player_flowers(l(dx, dy, dz))); 
