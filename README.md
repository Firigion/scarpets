...
# Skip night
Load with `/script load skip-night`. A tiny app to skip the night without the need for a bed bot. It comes in two modes: bed mode and command mode. You can toggle between them by edditing the configuration inside the app. Bed mode will skip the night as long as tehre is a bed with `skip` in its name placed somewhere in the overworld, no need for it to be in a loaded chunk. Command mode will not track beds, but creates a command `/skip-night toggle` to toggle between skipping on and off.
